...
void puppet_controller_4_cg_api(const mxArray *const prhs[24], int32_T nlhs,
                                const mxArray *plhs[2])
{
  ...
  cvec = (real_T(*)[4])mxMalloc(sizeof(real_T[4]));
  ...
  KD = e_emlrt_marshallIn(&st, emlrtAlias(prhs[17]), "KD");
  ...
  puppet_controller_4_cg(*q, *dq, *qd, *dqd, *ddqd, d, m, mm, hm, rm, r, kb, ks,
                         bb, bs, L0, *Kp, *KD, kc, ka, offset, contact,
                         conv_pcc, conv_motor, *tau, *cvec);
  /* Marshall function outputs */
  plhs[0] = emlrt_marshallOut(*tau);
  if (nlhs > 1) {
    plhs[1] = b_emlrt_marshallOut(*cvec);
  }
}

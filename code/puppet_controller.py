...
def puppet_controller(q,dq,qd,dqd,ddqd,d,hp,m,mm,hm,rm,r,N,kb,ks,bb,bs,L0,e,Kp,KD,kc,Ka,c_offset,Lm=0.013,contact=1):
    ...
    # permultiply C by a 16x16 square matrix but change the indices corresponding to the xms by 2 instead of 1
    # same as Kp diagonal matrix and you change the gains 
        # that makes sense 
    Cp = np.diag([1,1,1,1,3,1,1,1,2,1,1,1,2,1,1,1])
    GG = (Cp@C + M.dot(ddqd) + K.dot(qd) + D.dot(dqd) + (Fc*contact) + Kp.dot((qd-q)) + KD*(dqd - dq))

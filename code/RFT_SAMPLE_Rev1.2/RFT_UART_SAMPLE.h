...
public:
	...
	BOOL	openPort( char *devName, BYTE bPort = 0, DWORD rate = B115200 );
	BOOL	closePort( void );
	...
	std::array<float, 6> force_out;

	// std::array<float, 6> getForce() {
    // std::array<float, 6> result;
    // memcpy(result.data(), force_out, 6*sizeof(float));
    // return result;
  	// }

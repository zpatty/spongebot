    ...
    double x[3], double cq[4])
{
  ...
  for (r3 = 0; r3 < 16; r3++) {
    ...
    b_d += C[r3];
    b_C[r3] = b_d;
    c_C[r3] = (b_d + dc_dtheta) + d1;
  }
  b_KD[0] = 0.0 * contact;
  b_KD[4] = 0.0 * contact;
  b_KD[8] = 0.0 * contact;
  b_KD[12] = 0.0 * contact;
  b_KD[1] = Fci[0] * contact;
  b_KD[5] = Fci[3] * contact;
  b_KD[9] = Fci[6] * contact;
  b_KD[13] = Fci[9] * contact;
  b_KD[2] = Fci[1] * contact;
  b_KD[6] = Fci[4] * contact;
  b_KD[10] = Fci[7] * contact;
  b_KD[14] = Fci[10] * contact;
  b_KD[3] = Fci[2] * contact;
  b_KD[7] = Fci[5] * contact;
  b_KD[11] = Fci[8] * contact;
  b_KD[15] = Fci[11] * contact;
  for (b_i = 0; b_i < 16; b_i++) {
  ...
  for (b_i = 0; b_i < 16; b_i++) {
    ...
    for (i1 = 0; i1 < 16; i1++) {
      ibcol = b_i + (i1 << 4);
      b_d += Kp[ibcol] * b_K[i1];
      d1 += KD[ibcol] * b_dqd[i1];
    }
    b_d += c_C[b_i] + b_KD[b_i];
    b_KD[b_i] = d1;
    b_d += d1;
    b_C[b_i] = b_d;
  }
  ...
  KDr[136] = 0.0;
  KDr[204] = 0.0;
  memcpy(&c_C[0], &q[0], 16U * sizeof(double));
  J_r(c_C, L0, d, hm, J, x);
  for (b_i = 0; b_i < 3; b_i++) {
    for (i1 = 0; i1 < 16; i1++) {
  ...
  for (b_i = 0; b_i < 16; b_i++) {
    ...
    b_KD[b_i] = b_C[b_i] + b_d;
  }
  for (b_i = 0; b_i < 16; b_i++) {
    b_d = 0.0;
    for (i1 = 0; i1 < 16; i1++) {
      b_d += conversion[b_i + (i1 << 4)] * b_KD[i1];
    }
    tau_r[b_i] = b_d;
  }
}

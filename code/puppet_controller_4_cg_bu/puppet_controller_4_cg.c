...
#include "MC_4_cg.h"
#include "puppet_controller_4_cg_data.h"
                            ...
                            double cq[4])
{
  static const signed char inds[12] = {2,  3,  4,  6,  7,  8,
                                       10, 11, 12, 14, 15, 16};
  static const signed char b_iv[9] = {0, 1, 0, -1, 0, 0, 0, 0, 1};
  double A[256];
  double D[256];
  double K[256];
  double M[256];
  ...
  /*  Fci = cell(1,4); */
  /*  Ai = cell(1,4); */
  b_Ki_tmp = -d * 0.49999999999999994;
  for (i = 0; i < 4; i++) {
    double Aq[9];
    double Ki[9];
    double Kpi[9];
    ...
    for (b_i = 0; b_i < 3; b_i++) {
      ...
      for (A_tmp = 0; A_tmp < 3; A_tmp++) {
        ...
        c_Ki_tmp = b_i + 3 * A_tmp;
        dc_dL += del * Ki[c_Ki_tmp] * b_del_tmp;
        del_tmp += ((theta * Ki[3 * A_tmp] + d1 * Ki[ibcol]) +
                    dc_dtheta * Ki[ibtile]) *
                   b_del_tmp;
        b_y_tmp[c_Ki_tmp] = (theta * Kpi[3 * A_tmp] + d1 * Kpi[ibcol]) +
                            dc_dtheta * Kpi[ibtile];
        b_d += del * Kpi[c_Ki_tmp] * dv[A_tmp];
      }
      Fci[b_i + 3 * i] =
    ...
    for (b_i = 0; b_i < 3; b_i++) {
      ...
      for (A_tmp = 0; A_tmp < 3; A_tmp++) {
        Kpi[b_i + 3 * A_tmp] =
            (b_d * (double)iv[3 * A_tmp] + d1 * (double)iv[3 * A_tmp + 1]) +
            dc_dtheta * (double)iv[3 * A_tmp + 2];
      }
      Ki[3 * b_i + 2] = -1.0;
    }
    for (b_i = 0; b_i < 3; b_i++) {
  ...
  for (j = 0; j < 15; j++) {
    ...
    for (ibtile = 0; ibtile <= mmj_tmp; ibtile++) {
      ...
      if (dc_dtheta != 0.0) {
        b_i = ibcol + 18;
        A_tmp = (ibcol - j) + 32;
        for (c_Ki_tmp = b_i; c_Ki_tmp <= A_tmp; c_Ki_tmp++) {
          A[c_Ki_tmp - 1] += A[((b_tmp + c_Ki_tmp) - ibcol) - 17] * -dc_dtheta;
        }
      }
      ibcol += 16;
    }

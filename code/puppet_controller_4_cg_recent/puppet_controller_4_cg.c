 ...
 * File: puppet_controller_4_cg.c
 *
 * MATLAB Coder version            : 5.6
 * C/C++ source code generated on  : 18-Aug-2023 14:11:36
 */
                            ...
                            double conv_pcc, double conv_motor, double tau[16])
{
  ...
  for (i = 0; i < 4; i++) {
    ...
    if ((b_d < 1.0E-5) && (d1 < 1.0E-5)) {
      ...
      dc_dL = sin(theta / 6.0);
      c = 2.0 * (b_sig / theta - d) * dc_dL;
      dc_dtheta = -(cos(theta / 6.0) * (2.0 * d - 2.0 * b_sig / theta)) / 6.0 -
                  2.0 * dc_dL * b_sig / (theta * theta);
    ...
    Dq = dc_dtheta * dc_dx;
    Dq_tmp = dc_dtheta * dc_dy;
    del = dc_dtheta * dc_dL;
    for (b_i = 0; b_i < 3; b_i++) {
      ibtile = (Ki_tmp[b_i] - 1) << 4;
      Ki[3 * b_i] = K[(Ki_tmp_tmp + ibtile) - 1];
      ibcol = b_i + 3 * i;
      Kpi_tmp = (inds[ibcol] - 1) << 4;
      Kpi[3 * b_i] = Kp[(Ki_tmp_tmp + Kpi_tmp) - 1];
      dc_dtheta = qdp[ibcol];
      Fci_tmp[3 * b_i] = Dq * dc_dtheta;
      ibcol = 3 * b_i + 1;
      Ki[ibcol] = K[(b_Ki_tmp_tmp + ibtile) - 1];
      Kpi[ibcol] = Kp[(b_Ki_tmp_tmp + Kpi_tmp) - 1];
      Fci_tmp[ibcol] = Dq_tmp * dc_dtheta;
      ibcol = 3 * b_i + 2;
      Ki[ibcol] = K[(c_Ki_tmp_tmp + ibtile) - 1];
      Kpi[ibcol] = Kp[(c_Ki_tmp_tmp + Kpi_tmp) - 1];
      Fci_tmp[ibcol] = del * dc_dtheta;
    }
    for (b_i = 0; b_i < 3; b_i++) {
      b_d = 0.0;
      d1 = 0.0;
      ...
      theta = Fci_tmp[b_i];
      dc_dL = Fci_tmp[b_i + 3];
      del_tmp = Fci_tmp[b_i + 6];
      for (A_tmp = 0; A_tmp < 3; A_tmp++) {
        b_del_tmp = qdp[A_tmp + 3 * i];
        ibcol = b_i + 3 * A_tmp;
        d1 += b_sig * Ki[ibcol] * b_del_tmp;
        dc_dtheta += ((theta * Ki[3 * A_tmp] + dc_dL * Ki[3 * A_tmp + 1]) +
                      del_tmp * Ki[3 * A_tmp + 2]) *
                     b_del_tmp;
        b_d += b_sig * Kpi[ibcol] * b_del_tmp;
      }
      sig[b_i] = ((d1 + dc_dtheta) - b_d) - qp[b_i + 3 * i];
    }
    for (b_i = 0; b_i < 3; b_i++) {
      dc_dtheta = qp[b_i + 3 * i];
      Fci_tmp[3 * b_i] = Dq * dc_dtheta;
      Fci_tmp[3 * b_i + 1] = Dq_tmp * dc_dtheta;
      Fci_tmp[3 * b_i + 2] = del * dc_dtheta;
    }
    for (b_i = 0; b_i < 3; b_i++) {
      b_d = 0.0;
      d1 = Fci_tmp[b_i];
      dc_dtheta = Fci_tmp[b_i + 3];
      theta = Fci_tmp[b_i + 6];
      for (A_tmp = 0; A_tmp < 3; A_tmp++) {
        b_d += ((d1 * Kpi[3 * A_tmp] + dc_dtheta * Kpi[3 * A_tmp + 1]) +
                theta * Kpi[3 * A_tmp + 2]) *
               qp[A_tmp + 3 * i];
      }
      Fci[b_i + 3 * i] = -ka * sig[b_i] - b_d;
    }
  ...
  for (j = 0; j < 15; j++) {
    ...
    for (ibtile = 0; ibtile <= mmj_tmp; ibtile++) {
      ...
      if (dc_dtheta != 0.0) {
        b_i = ibcol + 18;
        A_tmp = (ibcol - j) + 32;
        for (Kpi_tmp = b_i; Kpi_tmp <= A_tmp; Kpi_tmp++) {
          A[Kpi_tmp - 1] += A[((b_tmp + Kpi_tmp) - ibcol) - 17] * -dc_dtheta;
        }
      }
      ibcol += 16;
    }
...
/*
 * File trailer for puppet_controller_4_cg.c
 *
 * [EOF]
 */

...
while 1:
    ...
    if key_input == chr(SPACE_ASCII_VALUE):
        ...
        while 1:
            if kbhit():
                ...
                break
            # elif np.linalg.norm(q) > 100:
            #     # for i in range(len(Mod1)):
            #     #        Mod1[i].disable_torque() 
            #     # break
            #     M1.send_torque_cmd(0)
            #     M2.send_torque_cmd(0)
            #     M3.send_torque_cmd(0)
            #     if np.linalg.norm(th1_0) > 100:
            #         th1_0 = [0.4970097752749477, 6.864564025788244, 1.3913205746122765]
            #         print("reset th0 to low")
            #     else:
            #         th1_0 = [6588396.755224174, 6.864564025788244, 1.3913205746122765]
            #         print("reset th0 to high")
            #     t = get_time()
            ...
            #     mod_lens = [mod_clock] + l1
            #     log_lens(mod_lens, False)
            #     q = grab_q(l1, s, d)
            #     print(f"norm(q): {np.linalg.norm(q)}\n")
            else:
                # grab current time
                t = get_time()
    ...
    elif key_input == chr(FKEY_ASCII_VALUE):
